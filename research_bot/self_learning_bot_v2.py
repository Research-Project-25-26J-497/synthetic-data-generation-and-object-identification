import rclpy
from rclpy.node import Node
from sensor_msgs.msg import Image, LaserScan
from geometry_msgs.msg import TwistStamped
from nav_msgs.msg import Odometry
from rclpy.qos import QoSProfile, ReliabilityPolicy
from cv_bridge import CvBridge
import cv2
from ultralytics import YOLO
import numpy as np
import math
import time
import os
import shutil
import yaml

class SelfLearningBotV2(Node):
    def __init__(self):
        super().__init__('self_learning_bot_v2')
        
        # --- CONFIGURATION ---
        self.batch_size = 30 
        self.epochs = 5      
        self.base_path = os.path.expanduser("~/ros2_ws/auto_dataset")
        
        self.train_img_dir = os.path.join(self.base_path, "train/images")
        self.train_lbl_dir = os.path.join(self.base_path, "train/labels")
        self.setup_directories()
        
        # MODEL MANAGEMENT
        self.base_model_path = 'yolov8n.pt'
        self.current_model_path = self.base_model_path
        self.model = YOLO(self.current_model_path)
        self.generation = 0 
        
        # WATCHDOG (The Safety Net)
        self.last_detection_time = time.time()
        self.watchdog_limit = 60.0 # Seconds before reverting
        
        # SENSORS
        qos_profile = QoSProfile(reliability=ReliabilityPolicy.BEST_EFFORT, depth=10)
        self.create_subscription(LaserScan, '/scan', self.scan_callback, qos_profile)
        self.create_subscription(Image, '/camera/image_raw', self.image_callback, 10)
        self.create_subscription(Odometry, '/odom', self.odom_callback, 10)
        self.publisher = self.create_publisher(TwistStamped, '/cmd_vel', 10)
        self.bridge = CvBridge()
        
        # STATE
        self.state = "CRUISE"
        self.lidar_ranges = []
        self.history = []
        self.current_x = 0.0
        self.current_y = 0.0
        self.frame_counter = 0
        self.front_dist = 10.0
        
        # DATA COLLECTION
        self.collected_count = 0
        self.last_save_time = 0
        self.turn_cooldown = 0
        self.target_turn_direction = 0.0
        self.obstacle_detected = False
        self.memory_cooldown = 0
        
        # PHYSICS
        self.current_speed = 0.0
        self.acceleration = 0.005

        self.get_logger().info(f"SELF-LEARNING V2: Watchdog Active ({self.watchdog_limit}s limit)")

    def setup_directories(self):
        if not os.path.exists(self.train_img_dir): os.makedirs(self.train_img_dir)
        if not os.path.exists(self.train_lbl_dir): os.makedirs(self.train_lbl_dir)
        
        yaml_content = {
            'path': self.base_path,
            'train': 'train/images',
            'val': 'train/images', 
            'names': {0: 'object'}
        }
        with open(os.path.join(self.base_path, 'data.yaml'), 'w') as f:
            yaml.dump(yaml_content, f)

    def odom_callback(self, msg):
        self.current_x = msg.pose.pose.position.x
        self.current_y = msg.pose.pose.position.y

    def scan_callback(self, msg):
        ranges = np.array(msg.ranges)
        ranges[ranges == float('inf')] = 10.0
        self.lidar_ranges = ranges
        front_indices = np.concatenate((np.arange(0, 30), np.arange(330, 360)))
        self.front_dist = np.min(ranges[front_indices])
        self.obstacle_detected = self.front_dist < 0.55

    def find_largest_gap(self):
        if len(self.lidar_ranges) == 0: return 0.0
        gaps = [np.mean(self.lidar_ranges[0:90]), np.mean(self.lidar_ranges[90:180]),
                np.mean(self.lidar_ranges[180:270]), np.mean(self.lidar_ranges[270:360])]
        best = gaps.index(max(gaps))
        return 1.0 if best <= 1 else -1.0

    def check_memory(self):
        if self.memory_cooldown > 0:
            self.memory_cooldown -= 1
            return False
        if len(self.history) < 200: return False
        for (px, py) in self.history[:-150]:
            if math.sqrt((self.current_x - px)**2 + (self.current_y - py)**2) < 0.8:
                return True
        return False

    def train_model(self):
        self.get_logger().warn("STARTING TRAINING SESSION...")
        try:
            results = self.model.train(
                data=os.path.join(self.base_path, 'data.yaml'),
                epochs=self.epochs,
                imgsz=640,
                batch=4,
                project=os.path.join(self.base_path, 'runs'),
                name=f'gen_{self.generation}',
                verbose=True
            )
            
            new_weights = os.path.join(self.base_path, f'runs/gen_{self.generation}/weights/best.pt')
            
            if os.path.exists(new_weights):
                self.get_logger().info("TRAINING SUCCESS! Updating brain.")
                self.current_model_path = new_weights
                self.model = YOLO(new_weights) 
                self.generation += 1
                self.collected_count = 0 
                
                # Reset Watchdog immediately so we don't revert instantly
                self.last_detection_time = time.time()
            else:
                self.get_logger().error("Weights not found.")
                
        except Exception as e:
            self.get_logger().error(f"TRAINING FAILED: {e}")
        
        self.state = "CRUISE"

    def revert_model(self):
        """ The Safety Net: Go back to the smart teacher """
        self.get_logger().error("WATCHDOG TRIGGERED: Model Collapse detected! Reverting to Base Model.")
        self.current_model_path = self.base_model_path
        self.model = YOLO(self.base_model_path)
        self.last_detection_time = time.time() # Reset timer
        # We don't reset 'generation' count so we know we tried.

    def save_training_data(self, frame, results):
        timestamp = int(time.time() * 1000)
        img_path = os.path.join(self.train_img_dir, f"{timestamp}.jpg")
        cv2.imwrite(img_path, frame)
        
        lbl_path = os.path.join(self.train_lbl_dir, f"{timestamp}.txt")
        with open(lbl_path, 'w') as f:
            for box in results[0].boxes:
                x, y, w, h = box.xywhn[0].tolist()
                f.write(f"0 {x} {y} {w} {h}\n")
        self.collected_count += 1

    def smooth_velocity(self, cmd, desired):
        if desired < 0: self.current_speed = desired
        else:
            if self.current_speed < desired: self.current_speed += self.acceleration
            elif self.current_speed > desired: self.current_speed -= self.acceleration
        self.current_speed = max(-0.15, min(self.current_speed, 0.15))
        cmd.twist.linear.x = self.current_speed

    def image_callback(self, msg):
        try:
            raw_frame = self.bridge.imgmsg_to_cv2(msg, "bgr8")
            
            cmd = TwistStamped()
            cmd.header.stamp = self.get_clock().now().to_msg()
            cmd.header.frame_id = 'base_link'
            
            if self.state == "TRAINING": return

            # --- ADAPTIVE CONFIDENCE ---
            # If using Base Model (Gen 0), strict (0.25).
            # If using Student Model (Gen > 0), lenient (0.1) to find *something*.
            conf_thresh = 0.25 if self.current_model_path == self.base_model_path else 0.1
            
            results = self.model(raw_frame, verbose=False, conf=conf_thresh)
            
            # --- WATCHDOG CHECK ---
            if len(results[0].boxes) > 0:
                self.last_detection_time = time.time() # Pet the watchdog
            else:
                # If blind for too long, revert
                time_blind = time.time() - self.last_detection_time
                if time_blind > self.watchdog_limit:
                    self.revert_model()
            
            # AUTO-LABEL
            if len(results[0].boxes) > 0 and (time.time() - self.last_save_time > 1.0):
                self.save_training_data(raw_frame, results)
                self.last_save_time = time.time()
                
                if self.collected_count >= self.batch_size:
                    self.target_speed = 0.0
                    self.smooth_velocity(cmd, 0.0)
                    self.publisher.publish(cmd) 
                    self.state = "TRAINING"
                    self.train_model() 
                    return

            # --- NAVIGATION ---
            self.frame_counter += 1
            if self.frame_counter % 10 == 0: self.history.append((self.current_x, self.current_y))

            target_speed = 0.15
            turn = 0.0
            status = "CRUISE"
            color = (0, 255, 0)
            
            if self.front_dist < 0.30:
                target_speed = -0.15
                status = "TOO CLOSE!"
                color = (0, 0, 255)
            elif self.turn_cooldown > 0:
                self.turn_cooldown -= 1
                target_speed = 0.0
                turn = 0.6 * self.target_turn_direction
                status = "LOCKED TURN"
                color = (255, 165, 0)
            elif self.obstacle_detected:
                self.turn_cooldown = 15
                self.target_turn_direction = self.find_largest_gap()
                target_speed = 0.0
                turn = 0.6 * self.target_turn_direction
                status = "AVOIDING"
                color = (0, 255, 255)
            elif self.check_memory():
                self.turn_cooldown = 30
                self.target_turn_direction = self.find_largest_gap()
                status = "LOOP ESCAPE"
                color = (255, 0, 255)
            
            self.target_speed = target_speed
            cmd.twist.angular.z = float(turn)
            self.smooth_velocity(cmd, target_speed)
            self.publisher.publish(cmd)
            
            # --- VISUALS ---
            frame = cv2.resize(raw_frame, (0, 0), fx=1.5, fy=1.5)
            
            # Draw boxes
            for box in results[0].boxes:
                x1, y1, x2, y2 = box.xyxy[0].cpu().numpy()
                x1, y1, x2, y2 = x1*1.5, y1*1.5, x2*1.5, y2*1.5
                cv2.rectangle(frame, (int(x1), int(y1)), (int(x2), int(y2)), (0, 255, 0), 2)
            
            # Watchdog Status Bar
            time_blind = int(time.time() - self.last_detection_time)
            wd_color = (0, 255, 0) if time_blind < 30 else (0, 0, 255)
            cv2.putText(frame, f"Gen: {self.generation} | Data: {self.collected_count}/{self.batch_size}", 
                       (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
            cv2.putText(frame, f"Blind: {time_blind}s (Limit: {int(self.watchdog_limit)})", 
                       (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, wd_color, 2)
            
            if self.current_model_path == self.base_model_path and self.generation > 0:
                cv2.putText(frame, "REVERTED TO BASE", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

            cv2.imshow("Self-Learning Bot V2", frame)
            cv2.waitKey(1)

        except Exception as e:
            self.get_logger().error(f"Error: {e}")

def main(args=None):
    rclpy.init(args=args)
    node = SelfLearningBotV2()
    try:
        rclpy.spin(node)
    except KeyboardInterrupt:
        pass
    finally:
        node.destroy_node()
        rclpy.shutdown()
        cv2.destroyAllWindows()

if __name__ == '__main__':
    main()
