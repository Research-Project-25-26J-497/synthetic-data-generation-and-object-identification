...
class DatasetCollector(Node):
    def __init__(self):
        ...
        self.get_logger().info(f"DATASET COLLECTOR: Saving images to {self.save_dir}")
    ...
    def image_callback(self, msg):
        try:
            ...
            # SAVE RAW IMAGE (Clean, no boxes)
            current_time = time.time()
            # Only save if 1 second has passed AND we are seeing objects
            if current_time - self.last_save_time > 1.0:
                # Run detection just to check if anything is there
            ...
            # PREPARE DISPLAY IMAGE (With boxes and text)
            ...
            # DISPLAY SAVE COUNT
            cv2.putText(frame, f"Saved Images: {self.images_saved}", (320, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
