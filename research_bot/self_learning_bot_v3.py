...
class SelfLearningBotV3(Node):
    def __init__(self):
        super().__init__('self_learning_bot_v3')
        ...
        self.get_logger().info(f"V3 ACTIVE: Hard-Stop Logic Enabled.")
    ...
    def hard_stop(self):
        """ Force the robot to freeze immediately """
        self.get_logger().warn("INITIATING HARD STOP FOR TRAINING...")
        ...
        # Publish multiple times to ensure Gazebo receives it
        for _ in range(5):
            self.publisher.publish(stop_cmd)
            time.sleep(0.05)
            
        # Reset internal speed tracker so we start fresh later
        self.current_speed = 0.0
        
        # Wait for physics to actually settle
        time.sleep(2.0)

    def train_model(self):
        # 1. STOP THE ROBOT FIRST
        self.hard_stop()
        
        # 2. START TRAINING
        self.get_logger().warn("ROBOT STOPPED. STARTING TRAINING...")
        try:
    ...
    def image_callback(self, msg):
        try:
            ...
            if len(results[0].boxes) > 0:
                ...
                if time.time() - self.last_detection_time > self.watchdog_limit:
                    self.revert_model()
            ...
            if len(results[0].boxes) > 0 and (time.time() - self.last_save_time > 1.0):
                ...
                if self.collected_count >= self.batch_size:
                    # CHANGE STATE FIRST to prevent further movement logic
                    self.state = "TRAINING"
                    # CALL THE FUNCTION THAT HANDLES STOPPING
                    self.train_model() 
                    return
            ...
            cv2.putText(frame, f"Blind: {time_blind}s", 
                       (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, wd_color, 2)
            
            cv2.imshow("Self-Learning Bot V3", frame)
            cv2.waitKey(1)
